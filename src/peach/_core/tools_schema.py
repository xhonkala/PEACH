...
TOOL_SCHEMAS: dict[str, ToolSchema] = {
    ...
    "tl.feature_simplex_regression": ToolSchema(
        ...
        Parameter("random_seed", ParamType.INTEGER, "int Seed for bootstrap and permutation RNGs. Vary to check stability.", required=False, default=42),
        Parameter("copy", ParamType.BOOLEAN, "bool If True, operate on a copy of adata.", required=False, default=False),
        ],
        returns="dict",
    ),
    ...
    "tl.flow_bifurcation": ToolSchema(
        ...
        description="Velocity-Jacobian divergence and eigenvalue analysis along the flow trajectory. Returns divergence [n_timepoints, n_cells], bifurcation_score [n_cells], eigenvalue_real/imag [n_timepoints, n_cells, dim]. n_saddle_points has been removed (was mathematically meaningless at non-fixed-points).",
        parameters=[
        Parameter("adata", ParamType.ADATA_REF, "AnnData", required=True, default=None),
        Parameter("flow_result", ParamType.OBJECT, "dict Output of flow_within.", required=True, default=None),
        Parameter("flow_model", ParamType.STRING, "FlowModel The trained FlowModel. Must match flow_result.", required=True, default=None),
        ...
        Parameter("evaluation_points", ParamType.STRING, "np.ndarray or None Points to evaluate. Default: source cells from flow_result.", required=False, default=None),
        ],
        returns="dict",
    ),
    "tl.flow_feature_graph": ToolSchema(
        name="tl.flow_feature_graph",
        description="DEPRECATED — use flow_jacobian(per_cell_features=True). Raises NotImplementedError.",
        parameters=[
        Parameter("adata", ParamType.ADATA_REF, "AnnData", required=True, default=None),
        Parameter("flow_result", ParamType.OBJECT, "dict", required=True, default=None),
        Parameter("flow_model", ParamType.STRING, "FlowModel", required=True, default=None),
        ],
        returns="NotImplementedError",
    ),
    "tl.flow_gene_alignment": ToolSchema(
        name="tl.flow_gene_alignment",
        description="Cosine alignment of PCA-space flow velocity with gene loading vectors. Always uses cosine similarity. adata must be the same object used in flow_within (obs_names validated). BH FDR applied over full gene family.",
        parameters=[
        Parameter("adata", ParamType.ADATA_REF, "AnnData", required=True, default=None),
        Parameter("flow_result", ParamType.OBJECT, "FlowWithinResult — must contain source_obs_names for validation.", required=True, default=None),
        Parameter("t", ParamType.FLOAT, "float or None Time point for instantaneous velocity. Requires return_model=True in flow_within. Default: None (use full-trajectory displacement).", required=False, default=None),
        ...
        Parameter("n_permutations", ParamType.INTEGER, "int Permutations for null distribution. Default: 0 (disabled).", required=False, default=0),
        Parameter("null_type", ParamType.STRING, "str Null model(s): 'rotation' (omnibus+rank), 'shuffle' (per-gene), or 'both'. Rank null only runs when rotation null is available. Default: 'both'.", required=False, default='both'),
        Parameter("null_mode", ParamType.STRING, "str or None Alias for null_type. Default: None.", required=False, default=None),
        Parameter("per_cell", ParamType.BOOLEAN, "bool Compute per-cell per-gene cosine alignment for top n_top_features genes. Default: True.", required=False, default=True),
        Parameter("n_top_features", ParamType.INTEGER, "int Max genes in per-cell alignment matrix. Default: 2500.", required=False, default=2500),
        Parameter("normalize", ParamType.BOOLEAN, "DEPRECATED — ignored, always cosine. Will be removed in v0.6.", required=False, default=True),
    ...
    "tl.flow_jacobian": ToolSchema(
        ...
        description="Flow map Jacobian ∂φ_t/∂x₀ via coupled ODE (midpoint, n_steps=100). Gene selection for per_cell_expansion is fixed from the last timepoint so multi-t stacking is consistent. adata obs_names validated against flow_result. BH FDR over full gene family.",
        parameters=[
        Parameter("adata", ParamType.ADATA_REF, "AnnData", required=True, default=None),
        Parameter("flow_result", ParamType.OBJECT, "FlowWithinResult — must contain source_obs_names for validation.", required=True, default=None),
        Parameter("flow_model", ParamType.STRING, "FlowModel The trained FlowModel. Must match flow_result.", required=True, default=None),
        Parameter("t", ParamType.FLOAT, "float or list of float Time point(s) in (0,1]. Single float → no leading dim. List → leading n_t dim + 'timepoints' key. Gene index fixed across timepoints. Default: 0.5.", required=False, default=0.5),
        Parameter("n_steps", ParamType.INTEGER, "int Midpoint integration steps. Default: 100.", required=False, default=100),
        Parameter("pca_loadings_key", ParamType.STRING, "str or None Key in adata.varm for PCA loadings. Default: 'PCs'.", required=False, default=None),
        Parameter("aggregate", ParamType.STRING, "str Aggregate per-cell Jacobians: 'mean' or 'median'. Default: 'mean'.", required=False, default='mean'),
        Parameter("per_cell_features", ParamType.BOOLEAN, "bool Compute per-cell per-gene expansion L_g^T J_c L_g for top n_top_features genes. Gene set fixed from last timepoint. Default: True.", required=False, default=True),
        Parameter("n_top_features", ParamType.INTEGER, "int Max genes for per-cell expansion matrix. Default: 2500.", required=False, default=2500),
        Parameter("n_permutations", ParamType.INTEGER, "int Permutations for expansion significance (last timepoint only). Default: 0.", required=False, default=0),
        Parameter("null_type", ParamType.STRING, "str Null model(s): 'rotation' (omnibus), 'shuffle' (per-gene), or 'both'. Default: 'both'.", required=False, default='both'),
        Parameter("null_mode", ParamType.STRING, "str or None Alias for null_type. Default: None.", required=False, default=None),
        Parameter("permutation_seed", ParamType.INTEGER, "int RNG seed for permutations. Default: 42.", required=False, default=42),
        ],
        returns="dict",
    ),
    ...
    "tl.flow_temporal_feature_graph": ToolSchema(
        ...
        description="DEPRECATED — use flow_jacobian(t=[...], per_cell_features=True). Raises NotImplementedError.",
        parameters=[
        Parameter("adata", ParamType.ADATA_REF, "AnnData", required=True, default=None),
        Parameter("flow_result", ParamType.OBJECT, "dict", required=True, default=None),
        Parameter("flow_model", ParamType.STRING, "FlowModel", required=True, default=None),
