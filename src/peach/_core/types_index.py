...
RETURN_TYPES: dict[str, tuple[str, list[str]]] = {
    ...
    "tl.feature_simplex_regression": ("dict", ['dict', 'Serialized SimplexRegressionResult. Stored in namespaced key:', "adata.uns['peach_simplex_regression_genes'] (when feature_matrix=None),", "adata.uns['peach_simplex_regression_pathways'] (when feature_matrix='pathway_scores'),", "or adata.uns['peach_simplex_regression_{feature_matrix}'] for other obsm keys.", 'Key fields: vertex_coefficients [n_features,K] raw betas;', 'vertex_contrasts [n_features,K] beta_j-mean(beta) summing to 0;', 'vertex_se [n_features,K] SE of contrasts;', 'vertex_pvalues [n_features,K] H0: beta_j=mean(beta);', 'vertex_pvalues_fdr [n_features,K]; f_pvalue [n_features] H0: all equal;', '...']),
    "tl.flow_between": ("dict", []),
    "tl.flow_bifurcation": ("dict", ['dict', 'Keys:', '- ``divergence``: [n_timepoints, n_cells] — trace(dv/dx), velocity Jacobian divergence', '- ``bifurcation_score``: [n_cells] — max |divergence| along trajectory', '- ``eigenvalue_real``: [n_timepoints, n_cells, dim] — real parts of dv/dx eigenvalues', '- ``eigenvalue_imag``: [n_timepoints, n_cells, dim] — imaginary parts', '- ``timepoints``: [n_timepoints]', 'NOTE: n_saddle_points removed (was mathematically meaningless at non-fixed-points).']),
    "tl.flow_feature_graph": ("NotImplementedError", ['DEPRECATED. Raises NotImplementedError. Use flow_jacobian(per_cell_features=True) instead.']),
    "tl.flow_gene_alignment": ("dict", ['dict', 'Keys:', '- ``alignment_scores``: [n_genes] — cosine similarity of gene loading with mean velocity', '- ``gene_names``: list[str]', '- ``top_aligned``: list[str] — top n_top genes most aligned with flow', '- ``top_opposed``: list[str] — top n_top genes most opposed to flow', '- ``velocity_mode``: str — "instantaneous" (t given + model) or "displacement"', '- ``per_cell_alignment``: [n_source, n_top_features] — optional, per_cell=True', '- ``alignment_pvalues``: [n_genes] — optional, n_permutations>0 + shuffle', '- ``alignment_pvalues_fdr``: [n_genes] — BH over full gene family', '- ``rotation_omnibus_pvalue``: float — optional, rotation null', '- ``alignment_pvalues_rank``: [n_genes] — optional, rank null (rotation only)', '- ``alignment_pvalues_rank_fdr``: [n_genes] — BH FDR for rank pvalues']),
    "tl.flow_jacobian": ("dict", [
        "dict — flow map Jacobian ∂φ_t/∂x₀ results (accumulated stretching).",
        "Scalar-t keys: 'jacobian_det' [n_cells], 'jac_logdet' [n_cells],",
        "'jac_det_sign' [n_cells], 'feature_expansion' [n_genes],",
        "'mean_jacobian' [dim,dim], 'phi' [n_cells,dim], 't' float.",
        "Optional: 'per_cell_expansion' [n_cells,n_top_feat],",
        "'per_cell_expansion_gene_names' list, 'per_cell_expansion_gene_indices'.",
        "Perm: 'expansion_pvalues', 'expansion_pvalues_fdr' [n_genes],",
        "'expansion_rotation_omnibus_pvalue' float.",
        "List-t: arrays gain leading n_t dim + 'timepoints' list[float].",
    ]),
    "tl.flow_significance": ("dict", ['dict with p_value, observed_stat, null_distribution']),
    "tl.flow_temporal_feature_graph": ("NotImplementedError", ['DEPRECATED. Raises NotImplementedError. Use flow_jacobian(t=[...], per_cell_features=True) instead.']),
    "tl.flow_within": ("dict", ['dict', 'Keys:', '- ``source_mask``: [n_cells] bool', '- ``target_mask``: [n_cells] bool', '- ``source_obs_names``: list[str] — obs_names of source cells; validated by flow_jacobian/flow_gene_alignment', '- ``transported``: [n_source, dim] — source cells transported to target by flow model', '- ``losses``: list[float] — training losses per epoch', '- ``mmd_before``: float', '- ``mmd_after``: float', '- ``pca_key``: str', '- ``name``: str or None', '- ``model``: FlowModel — optional, only when return_model=True', '- ``holdout_mmd``: float — optional, only when holdout_fraction>0']),
...
USE_GET_FOR = {
    ...
    "alignment_pvalues_rank",
    "alignment_pvalues_rank_fdr",
    "alignment_rank_null_mean",
    "alignment_rank_null_std",
    "bootstrap_ci_lower",
    "bootstrap_ci_upper",
