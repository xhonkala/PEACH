...
def ternary_facet(
    ...
    **kwargs
    ...
    fig.update_layout(
        ternary=dict(
            sum=1,
            aaxis=dict(title=f"A{i+1}", min=0, linewidth=1, linecolor="#333",
                       gridcolor="#eee", ticklen=3),
            baxis=dict(title=f"A{j+1}", min=0, linewidth=1, linecolor="#333",
                       gridcolor="#eee", ticklen=3),
            caxis=dict(title=f"A{k+1}", min=0, linewidth=1, linecolor="#333",
                       gridcolor="#eee", ticklen=3),
            bgcolor="white",
        ),
        font=dict(family="Arial, Helvetica, sans-serif", size=12),
        title=dict(text=f"Archetypes ({i+1}, {j+1}, {k+1})", x=0.02,
